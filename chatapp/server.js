...
io.on('connection', (socket) => {
  ...
  // Lets the other side know a call ring should stop (e.g. caller hung up before pickup)
  socket.on('cancel_call', ({ from, to }) => {
    const recipientSocketId = onlineUsers.get(to);
    if (recipientSocketId) io.to(recipientSocketId).emit('call_ended', { from });
  });
  ...
  socket.on('send_message', ({ from, to, text, attachment }) => {
    ...
    const data = readDb();
    // Recipient online right now? Then we can consider it "delivered" immediately.
    const recipientSocketId = onlineUsers.get(to);
    const message = {
      id: data.nextId++,
      ...
      status: recipientSocketId ? 'delivered' : 'sent' // sent -> delivered -> read
    };
  ...
  // Recipient has opened/viewed the conversation - mark their unread messages as read
  // and tell the original sender so their ticks can turn blue.
  socket.on('mark_read', ({ reader, otherUser }) => {
    if (!reader || !otherUser) return;
    const data = readDb();
    const convoId = conversationId(reader, otherUser);
    let changed = false;
    data.messages.forEach(m => {
      if (m.conversationId === convoId && m.to === reader && m.from === otherUser && m.status !== 'read') {
        m.status = 'read';
        changed = true;
      }
    });
    if (changed) {
      ...
      if (otherSocketId) {
        io.to(otherSocketId).emit('messages_read', { reader, otherUser });
      }
    }
  });
  ...
  socket.on('disconnect', () => {
    if (currentUser && onlineUsers.get(currentUser) === socket.id) {
