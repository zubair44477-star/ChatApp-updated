...
function connectSocket() {
  ...
  socket.on('new_message', (msg) => {
    ...
    if (currentChatWith && (msg.from === currentChatWith || msg.to === currentChatWith)) {
      appendMessage(msg);
      // If we're actively looking at this chat and the message is incoming, mark it read right away
      if (msg.from === currentChatWith && msg.to === myName) {
        socket.emit('mark_read', { reader: myName, otherUser: currentChatWith });
      }
    }
    loadContacts(); // refresh previews/order
  });

  // A message we sent just got delivered to the recipient's device
  socket.on('message_status', ({ id, status }) => {
    const bubble = document.querySelector(`.message-bubble[data-msg-id="${id}"]`);
    if (bubble) {
      const tickEl = bubble.querySelector('.msg-tick');
      if (tickEl) tickEl.outerHTML = tickIcon(status);
    }
  });

  // The other person just read our messages - flip all our ticks in this chat to blue
  socket.on('messages_read', ({ reader }) => {
    if (reader !== currentChatWith) return;
    document.querySelectorAll('.message-bubble.bubble-out .msg-tick').forEach(el => {
      el.outerHTML = tickIcon('read');
    });
  });
...
async function openChat(name) {
  ...
  // Tell the server (and the other person) that we've now seen their messages
  socket.emit('mark_read', { reader: myName, otherUser: name });
}
...
function tickIcon(status) {
  if (status === 'read') {
    return `<svg class="msg-tick tick-read" viewBox="0 0 16 11" width="16" height="11"><path fill="currentColor" d="M11.071.653a.457.457 0 0 0-.304-.102.493.493 0 0 0-.381.178l-6.19 7.636-2.405-2.272a.463.463 0 0 0-.336-.146.47.47 0 0 0-.336.146l-.43.43a.46.46 0 0 0 0 .655l3.066 3.066a.46.46 0 0 0 .655 0l6.836-8.428a.497.497 0 0 0-.038-.71l-.137-.453zm4.482 0a.457.457 0 0 0-.304-.102.493.493 0 0 0-.381.178l-6.19 7.636-.57-.538-.804.99 1.044.99a.46.46 0 0 0 .655 0l6.836-8.428a.497.497 0 0 0-.038-.71l-.248-.016z"/></svg>`;
  }
  if (status === 'delivered') {
    return `<svg class="msg-tick tick-delivered" viewBox="0 0 16 11" width="16" height="11"><path fill="currentColor" d="M11.071.653a.457.457 0 0 0-.304-.102.493.493 0 0 0-.381.178l-6.19 7.636-2.405-2.272a.463.463 0 0 0-.336-.146.47.47 0 0 0-.336.146l-.43.43a.46.46 0 0 0 0 .655l3.066 3.066a.46.46 0 0 0 .655 0l6.836-8.428a.497.497 0 0 0-.038-.71l-.137-.453zm4.482 0a.457.457 0 0 0-.304-.102.493.493 0 0 0-.381.178l-6.19 7.636-.57-.538-.804.99 1.044.99a.46.46 0 0 0 .655 0l6.836-8.428a.497.497 0 0 0-.038-.71l-.248-.016z"/></svg>`;
  }
  // sent (single tick)
  return `<svg class="msg-tick tick-sent" viewBox="0 0 16 11" width="13" height="9"><path fill="currentColor" d="M11.071.653a.457.457 0 0 0-.304-.102.493.493 0 0 0-.381.178l-6.19 7.636-2.405-2.272a.463.463 0 0 0-.336-.146.47.47 0 0 0-.336.146l-.43.43a.46.46 0 0 0 0 .655l3.066 3.066a.46.46 0 0 0 .655 0l6.836-8.428a.497.497 0 0 0-.038-.71l-.137-.453z"/></svg>`;
}
...
function appendMessage(msg) {
  ...
  if (isOut) bubble.dataset.msgId = msg.id;
  ...
  html += `<span class="msg-time">${formatTime(msg.timestamp)}${isOut ? tickIcon(msg.status || 'sent') : ''}</span>`;
...
const ICE_SERVERS = {
  iceServers: [
    ...
    // Public TURN relay fallback - without this, calls often fail to connect on
    // mobile data / restrictive WiFi, even though the phone "rings" fine.
    {
      urls: [
...
async function startCall(callType) {
  ...
  // Ask for mic/camera permission up front, before ringing. If the user blocks
  // permissions this is where it would fail - catching it here gives a clear
  // error instead of a call that silently never connects.
  try {
    ...
    alert('Could not access microphone/camera. Please allow permission and try again.');
    callPartner = null;
  ...
  // If nobody answers within 30s, stop ringing automatically.
  clearTimeout(ringTimeout);
  ringTimeout = setTimeout(() => {
    if (callPartner) {
      socket.emit('cancel_call', { from: myName, to: callPartner });
      alert(`${callPartner} didn't answer.`);
      endCallUI();
    }
  }, 30000);
}
...
function bindCallSocketEvents() {
  ...
  socket.on('call_accepted', async ({ from }) => {
    ...
    try {
      ...
      alert('Could not start the call. Please check your microphone/camera permissions.');
      endCallUI();
    }
  });
